...
namespace HJD.AccessServiceTask.Job.Search
{
    ...
    /// 生成搜索系统的相关索引库
    /// </summary>
    public class BuildQaIndexJob : BaseJob
    {
        public BuildQaIndexJob()
            : base("BuildQaIndexJob")
        {
            Log(string.Format("Start Job [BuildQaIndexJob]"));
            ...
            {
                Log("Job [BuildQaIndexJob] Error:" + ex.Message);
            }


            Log(string.Format("Stop Job [BuildQaIndexJob]"));
        }
        ...
        {
            var indexEngine = new IndexEngine(SearchType.QaHotel);
        ...
        {
            var docDataList = IndexEngine.GetIndexDocsById(SearchType.QaHotel, null);
