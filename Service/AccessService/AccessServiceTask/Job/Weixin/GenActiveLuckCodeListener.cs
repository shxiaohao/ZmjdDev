...
namespace HJD.AccessServiceTask.Job
{
    ...
    /// 监听“发布增加新抽奖码的队列任务”
    /// </summary>
    public class GenActiveLuckCodeListener : BaseJob
    {
        RabbitMqContext rbmqContext;
        static string queueKey = "PublishWeixinLuckCodeTask";

        public GenActiveLuckCodeListener()
            : base("GenActiveLuckCodeListener")
        {
            Log(string.Format("Start Job [GenActiveLuckCodeListener]"));
            ...
            {
                Log("Job [GenActiveLuckCodeListener] Error:" + ex.Message);
            }


            Log(string.Format("Stop Job [GenActiveLuckCodeListener]"));
        }
