...
{
    public class QaSearchEngine
    {
        #region 单例对象

        private static Dictionary<SearchType, QaSearchEngine> dic_Instance = new Dictionary<SearchType, QaSearchEngine>();

        public static QaSearchEngine GetInstance(SearchType t)
        {
            if (!dic_Instance.ContainsKey(t))
            {
                dic_Instance[t] = new QaSearchEngine(t);
            }
        ...
        //对象函数
        public QaSearchEngine(SearchType t)
        {
            InitSearchParams(t);
        }
        ...
        public static ICollection<WordInfo> GetSegment(string word)
        {
            if (NLPEngine.Segment == null) NLPEngine.Segment = new PanGuSegment();
            return NLPEngine.Segment.GetSegment(word);
        }



        /// <summary>
        /// 分析是否有周边类型的目的地，如果有，那么需要将目的地进行扩展
        /// </summary>
        /// <param name="wList"></param>
        /// <returns></returns>
        public static bool  Parse_IsAroundCity(ICollection<WordInfo> wList)
        {
            //如果城市后面跟了 “周边、周围、附近”那么认为是周边类型的目的地
