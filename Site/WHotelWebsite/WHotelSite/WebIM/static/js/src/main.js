;(function ( window, undefined ) {
	...
	var main = function ( config ) {
		...
		if( utils.root ) {
            ...
            config.tenantId = easemobim.config.tenantId;
            config.hide = true;
            config.to = window.easemobim.config.to ;//|| utils.convertFalse(utils.query('to'));
            config.appKey = window.easemobim.config.appKey;// || utils.convertFalse(decodeURIComponent(utils.query('appKey')));
            config.domain = window.easemobim.config.domain;// || config.domain || '//' + location.host;
            config.staticPath = window.easemobim.config.staticPath;//
            config.xmppServer = window.easemobim.config.xmppServer;// || utils.convertFalse(utils.query('xmppServer'));
            config.restServer = window.easemobim.config.restServer;// ||utils.convertFalse(utils.query('restServer'));
            config.originType = window.easemobim.config.originType;// || utils.convertFalse(utils.query('originType'));
            config.agentName = window.easemobim.config.agentName;// || utils.convertFalse(utils.query('agentName'));
            config.satisfaction = window.easemobim.config.sat;//|| utils.convertFalse(utils.query('sat'));
            config.resources = window.easemobim.config.resources;// || utils.convertFalse(utils.query('resources'));
            config.hideStatus = window.easemobim.config.hideStatus;//|| utils.convertFalse(utils.query('hideStatus'));
            //config.satisfaction = window.easemobim.config.sat || utils.convertFalse(utils.query('sat'));
            config.wechatAuth = window.easemobim.config.wechatAuth;//|| utils.convertFalse(utils.query('wechatAuth'));
            config.hideKeyboard = window.easemobim.config.hideKeyboard;//|| utils.convertFalse(utils.query('hideKeyboard'));
            config.ticket = window.easemobim.config.ticket === '' ? true : window.easemobim.config.ticket ;// || utils.query('ticket') === '' ? true : utils.convertFalse(utils.query('ticket'));//true default
            ...
            if ( !utils.query('user') ) {
                config.user = {
                    username: window.easemobim.config.user.name,// || utils.get('root' + config.tenantId + config.emgroup),
                    password: window.easemobim.config.user.password ,//||'',
                    token: ''
                };
            } else if ( !config.user || (config.user.username && config.user.username !== utils.query('user')) ) {
                config.user = {
                    username: window.easemobim.config.user.name,//|| '',
                    password: window.easemobim.config.user.password,//|| '',
                    token: ''
                };
            } 
		...
		webim.innerHTML = "\
			...
			<div id='EasemobKefuWebimChat' class='easemobWidgetWrapper" + (utils.root || !config.minimum ? "" : " em-hide")  + (utils.isMobile ? " easemobWidgetWrapper-mobile" : "") + "'>\
				<div id='easemobWidgetHeader' class='easemobWidgetHeader-wrapper bg-color border-color'  style=\"display:none\">\
