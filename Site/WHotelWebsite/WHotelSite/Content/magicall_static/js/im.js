...
; (function (window, undefined) {
    ...
    var leaveMessage = function () {
        ...
        utils.html(leaveMessage.dom, "\
            <h3>欢迎使用MagiCall！<BR/><BR/>现在是休息时间，我们的工作时间是每天早上9：00-晚上12：00，节假日同。<BR/><BR/>请留言您所需酒店的任意要求，我们将为尽力您一站式解决。为您省时、省力、省钱。 <BR/><BR/>举例：上海周边开车2小时之内的亲子酒店，1000元以内，有儿童泳池，12/12日入住1晚，需要1间。</h3>\
            <BR/>\
            <textarea spellcheck='false' placeholder='请输入留言'></textarea>\
    ...
    */
        ...
        return {
                    ...
                    Number(chatWrapper.getAttribute('data-history')) || api('getGroup', {
                        ...
                        if (msg && msg.data) {
                            ...
                            api('getHistory', {
                                ...
                                else if (msg && msg.data && msg.data.length == 0) { //如果没有历史纪录
                                    sentWelcomeMsg(me);
                                }
                            });
                        }
            ...
            , soundReminder: function () {
                ...
                if (window.HTMLAudioElement) {
                    ...
                    me.audio.src = '/Content/magicall_static/mp3/msg.m4a';
                    me.soundReminder = function () {
            ...
            , bindEvents: function () {
                ...
                utils.on(sendBtn, 'click', function () {
                    ...
                    afterUserSendMsg(me, content); //WHAPP
                    return false;
                });
            }
            ...
            , receiveMsg: function (msg, type, isHistory) {
                ...
                if (!isHistory) {
                    ...
                    if (dealWHAPPMsg(message, msg, me) > 0) //处理周末酒店自定义数据格式
                    {
                        return;
                    }
    ...
    Easemob.im.EmMessage.txt.prototype.get = function (isReceive) {
        ...
        var rethtml = "";
        if (startWith(this.value, "WHAPP:")) {
            rethtml = ShowWHAppMsg(this.id, this.value);
        }
        else {
            rethtml = [
                !isReceive ? "<div id='" + this.id + "' class='easemobWidget-right'>" : "<div class='easemobWidget-left'>",
                !isReceive ? "<div class='WebimUserHeadPhoto'><img src='" + userHeadPhoto + "' alt='UserHeadPhoto' /></div>" : "<div class='WebimKeFuHeadPhoto'><img src='" + kefuHeadPhoto + "' alt='KeFuHeadPhoto' /></div>",
    ...
    config.LOADING = !utils.isQQBrowserInAndroid && !(utils.getIEVersion() && utils.getIEVersion() == 9)//消息loading
        ...
        "c2.8,0,5.3,1,7.3,2.8'/><image src='/Content/magicall_static/img/loading.gif' width='20' style='margin-top:10px;' /></svg></div>"].join('')
        : "<img src='/Content/magicall_static/img/loading.gif' width='20' style='margin-top:10px;'/>";
    Easemob.im.EMOTIONS = {//表情集
        path: '/Content/magicall_static/img/faces/'
        , map: {
            '[):]': 'ee_1.png',
            '[:D]': 'ee_2.png',
            '[;)]': 'ee_3.png',
